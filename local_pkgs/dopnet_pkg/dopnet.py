...
from local_pkgs.dopnet_pkg import chem
...
def train(model, data_loader, optimizer, criterion, device):
    ...
    for host_feats, dop_feats, targets in data_loader:
        host_feats = host_feats.to(device)
...
def load_dataset(dataset_file_name, comp_idx, target_idx, max_dops, cond_idx=None, norm_target=False, doping_threshold=0.1):
    elem_feats = chem.load_elem_feats()
    data = numpy.array(pandas.read_excel(dataset_file_name))
    
    # Get initial shape
    initial_shape = data.shape
    print(f"Initial shape of dataset: {initial_shape}")
    
    # Extract targets and identify missing values
    targets = data[:, target_idx].astype(float).reshape(-1, 1)
    comps = data[:, comp_idx]
    
    # Find rows with NaN values in the target column
    missing_indices = numpy.isnan(targets).flatten()
    if missing_indices.any():
        print(f"Rows with missing target values: {numpy.where(missing_indices)[0].tolist()}")
    
    # Filter out rows with missing target values
    data = data[~missing_indices]
    targets = targets[~missing_indices]
    comps = comps[~missing_indices]
    
    # Get final shape after filtering
    final_shape = data.shape
    print(f"Final shape of dataset after dropping missing targets: {final_shape}")
    
    # Normalization for conditional indices if provided
    if cond_idx is not None:
        norm_conds = scale(data[:, cond_idx]) # essentially uses the StandardScaler
    ...
    if norm_target:
        return dataset, comps, target_mean, target_std
    else:
        return dataset, comps


def calc_atom_feats(elem_feats, comp, max_dops, doping_threshold):
    host_feats = list()
...
def get_dataset(list_data, max_dops):
    ...
    targets = torch.tensor(numpy.array(targets), dtype=torch.float).view(-1, 1)
