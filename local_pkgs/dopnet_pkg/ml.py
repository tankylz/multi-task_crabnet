...
def get_k_folds_by_index_ls(dataset: list, split_ls):
    """
    Created on top of the original DopNet code.

    Used to split the list of DopedMat objects by a list of lists (of indices).
    """
    included_indices = set()

    folds = list()
    for split in split_ls:
        fold = [dataset[idx] for idx in split]
        folds.append(fold)
        included_indices.update(split)

    # Check if all indices in the dataset were included
    if len(included_indices) != len(dataset):
        raise ValueError('Some data points were not included in the split')
