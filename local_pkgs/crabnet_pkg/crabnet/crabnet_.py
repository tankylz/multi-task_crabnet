...
from .kingcrab import SubCrab

# for backwards compatibility of imports
from .utils.data import get_data, groupby_formula  # noqa: F401
...
class CrabNet(nn.Module):
    ...
    def __init__(
        ...
        model_name: str = "UnnamedModel", # Name
        n_elements: Union[str, int] = "infer",
        classification: bool = False,
        target_scaling: Union[str, dict] = None, 
        verbose: bool = True,
        ...
        extra_enc_resolution: int = 5000,
        extra_enc_log_resolution: int = 5000,
        bias=False,
        emb_scaler: float = 1.0,
        pos_scaler: float = 1.0,
        pos_scaler_log: float = 1.0,
        extra_scaler: float = 1.0,
        extra_scaler_log: float = 1.0,
        ...
        mat_prop: Optional[Union[str, PathLike]] = None, # Name
        losscurve: bool = True, # Plot
        learningcurve: bool = True, # Plot
        save: bool = True, # Save Model
        ...
        min_trust : float, optional
            Sets a lower bound to prevent the optimizer from scaling the learning rate too aggressively, by default None
        ...
        num_properties: int, optional
            Number of properties to predict. By default 1. Values > 1 indicate multi-task learning.
        property_name_col: str, optional
            The name of the column containing the property names. Used in multi-task learning.
        property_one_hot_col: str, optional
            The name of the column containing the one-hot encoding of properties. Used in multi-task learning.
        target_scaling: str or dict, optional
            The method to scale the target values. By default None. If multi-task learning, it should be a dictionary. Possible values are 'yeo-johnson', 'box-cox', 'log10'.
        """

        if num_properties > 1:
            if property_name_col is None or property_one_hot_col is None:
                raise ValueError("Both property_name_col and property_one_hot_col must be provided for multi-task learning.")
        elif num_properties < 1:
            raise ValueError("num_properties must be >= 1.")

        if fudge != 0 and (target_scaling =='yeo-johnson' or target_scaling == 'box-cox'):
            raise ValueError(
                "Cannot use both fudge and power transform due to risk of NaN values when unscaling data. Choose one or the other."
            )
        
        if (extend_features is None) != (how_to_extend is None):
            raise ValueError("Both `extend_features` and `how_to_extend` must be specified together.")

        super().__init__() # initialize nn.Module with default parameters
        ...
        self.extra_scaler_log = extra_scaler_log
        ...
        self.property_name_col = property_name_col
        self.property_one_hot_col = property_one_hot_col
    ...
    def fit(
        ...
        """
        # used for concatenating extra features after the transformer and before the residual block
        if self.extend_features is not None and (self.how_to_extend == "concat_at_output"):
            self.d_extend = len(self.extend_features)
        else:
            self.d_extend = 0
        ...
        if self.model is None:
            self.model = SubCrab(
                ...
                extra_enc_resolution=self.extra_enc_resolution,
                extra_enc_log_resolution=self.extra_enc_log_resolution,
    ...
    def predict(
        ...
        if test_df is None and loader is None:
            ...
            if self.extend_features is not None:
                ...
                extra_features = None
            self.load_data(test_df, extra_features=extra_features, train=False)
        ...
        with torch.no_grad():
            for i, batch_df in enumerate(loader):
                # extract data
                X, y, formula, extra_features, one_hot, one_hot_arr = batch_df

                one_hot_arr = np.array(one_hot_arr)
                unique_props = np.unique(one_hot_arr)
                ...
                # predict
                output = self.model.forward(src, frac, extra_features=extra_features, property_one_hot=one_hot)
                prediction, uncertainty = output.chunk(2, dim=-1)

                if self.num_properties > 1:
                    for prop_name in unique_props:
                        prop_indices = np.where(one_hot_arr == prop_name)[0]
                        prediction[prop_indices] = self.scalers[prop_name].unscale(prediction[prop_indices])
                        uncertainty[prop_indices] = torch.exp(uncertainty[prop_indices]) * self.scalers[prop_name].std
                else:
    ...
    def load_data(
        ...
        batch_size: int = 2**9,
        train: bool = False,
        target_scaling: str = None, # allowed are 'yeo-johnson' , 'box-cox' , 'log10'
    ):
        ...
        # Extract one-hot encoded vectors
        if self.num_properties > 1:
            # Stack and convert to tensor
            one_hot_vectors = np.stack(data[self.property_one_hot_col].values)  # Stack arrays into 2D array
            one_hot_tensors = torch.tensor(one_hot_vectors, dtype=torch.float32)
            one_hot_names = list(data[self.property_name_col].values)
        else:
        ...
        data_loaders = EDM_CsvLoader(
            ...
            one_hot_tensors=one_hot_tensors,
            one_hot_names=one_hot_names,
        ...
        data_loader = data_loaders.get_data_loaders(inference=inference) # batch loading

        if train:
            y = data_loaders.main_data[1]  # Target data
            self.train_len = len(y)

            # Initialize scalers for each property
            if self.num_properties > 1:
                self.scalers = {}
                for i, prop_name in enumerate(data[self.property_name_col].unique()):
                    target_values = y[data[self.property_name_col] == prop_name]
                    if target_scaling[prop_name] == 'yeo-johnson' or target_scaling[prop_name] == 'box-cox':
                        pt = PowerTransformer(method=target_scaling[prop_name])
                        pt.fit(target_values.reshape(-1, 1))
                        lambda_ = pt.lambdas_[0]
                        self.scalers[prop_name] = PowerTorchTransformer(
                            target_values, lambda_, method=target_scaling[prop_name]
                        )
                    elif target_scaling[prop_name] == 'log10':
                        self.scalers[prop_name] = LogScaler(target_values, base=10)
                    else:
                        self.scalers[prop_name] = Scaler(target_values)
            else:
                # Single-target case
                if target_scaling in ['yeo-johnson', 'box-cox']:
                    pt = PowerTransformer(method=target_scaling)
                    pt.fit(y.reshape(-1, 1))
                    lambda_ = pt.lambdas_[0]
                    self.scaler = PowerTorchTransformer(y, lambda_, method=target_scaling)
                elif target_scaling == 'log10':
                    self.scaler = LogScaler(y, base=10)
                else:
    ...
    def _train(self):
        ...
        for data in self.train_loader:
            ...
            X, y, _, extra_features, one_hot, one_hot_arr = data

            one_hot_arr = np.array(one_hot_arr)
            unique_props = np.unique(one_hot_arr)

            # Scale each target property independently
            if self.num_properties > 1:
                for prop_name in unique_props:
                    # get the idx of all the datapoints of the prop_name
                    prop_indices = np.where(one_hot_arr == prop_name)[0]

                    # scale the target values of that property
                    y[prop_indices] = self.scalers[prop_name].scale(y[prop_indices])

            else:
                y = self.scaler.scale(y)
                
            # just plot the data for now
            # histogram_plot(y, 'Power Transformed Train')
    ...
    def _load_trainval_data(
        ...
        if val_data is not None:
            self.load_data(
                val_data, batch_size=batch_size, extra_features=extra_val_data, train=False
            )
    ...
    def save_network(self, model_name: str = None):
        ...
        if self.num_properties > 1:
            self.network = {
                ...
                "scaler_state": {key: value.state_dict() for key, value in self.scalers.items()},
    ...
    def load_model(
        ...
        self.load_data(data, batch_size=2**9, train=False, target_scaling=self.target_scaling)
